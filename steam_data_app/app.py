...
def main():
    ...
    ranking = df.T[latest_day].sort_values(ascending=False)[0:5]
    ...
    with ranking_container:
        st.header('Max User Top 5 Games on ' + latest_day)
    ...
    with transition_container:
        ...
        with col2_bottom:
            transition_top5_fig = px.line(df, x=df.index, y=ranking.keys(), markers=True, labels=dict(x="Date", y="Max Users", color='Games'))
